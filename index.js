...
const rules = {
  imageTag: Object.assign({}, markdown.defaultRules.image, {
    match: markdown.inlineRegex(/^<img\s+([^>]+)>/i),
    parse: (capture, parse, state) => {
      const attributes = {};
      const attrRegex = /(\w+)=["']([^"']+)["']/g;
      let match;
      while ((match = attrRegex.exec(capture[1])) !== null) {
    ...
    html: (node, output, state) => {
      ...
      return htmlTag("img", "", attributes, state);
    },
  }),
  ...
  codeBlock: Object.assign({}, markdown.defaultRules.codeBlock, {
    ...
    html: (node, output, state) => {
      ...
      return htmlTag(
        "pre",
        htmlTag("code", codeHtml, null, state),
        null,
        state
      );
    },
...
const htmlOutputSlack = markdown.htmlFor(
  markdown.ruleOutput(rulesSlackOnly, "html")
);
